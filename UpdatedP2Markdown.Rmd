...
set.seed(300)
ann_model <- train(as.factor(Drafted) ~ ., data = nfl, method = "nnet",
           metric = "Kappa",
           trControl = ctrl)
ann_model
ann_pred <- predict(ann_model, nfl)
CrossTable(x = nfl$Drafted, y = ann_pred, prop.chisq = F, dnn = c('actual yes', 'predicted yes'))
confusionMatrix(as.factor(nfl$Drafted), as.factor(ann_pred))
```
...
# customize train() with the control list and grid of parameters 
set.seed(300)
lm <- train(as.factor(Drafted) ~ ., data = nfl, method = "glmnet",
           metric = "Kappa",
           trControl = ctrl,
           family = "binomial")
lm
lm_pred <- predict(lm, nfl)
table(lm_pred, nfl$Drafted)
CrossTable(x = nfl$Drafted, y = lm_pred, prop.chisq = F, dnn = c('actual yes', 'predicted yes'))
confusionMatrix(as.factor(nfl$Drafted), as.factor(lm_pred))
```
...
knn <- train(as.factor(Drafted) ~ ., data = nfl, method = "knn", metric="Accuracy", trControl = training_control, tuneGrid = grid)
knn
knn_pred <- predict(knn, nfl)
CrossTable(x = nfl$Drafted, y = knn_pred, prop.chisq = F, dnn = c('actual yes', 'predicted yes'))
confusionMatrix(as.factor(nfl$Drafted), as.factor(knn_pred))
```
...
svm_Linear <- train(as.factor(Drafted) ~., data = nfl, method = "svmRadial",
                    trControl=ctrl,
...
# customize train() with the control list and grid of parameters 
svm_model <- train(as.factor(Drafted) ~ ., data = nfl, method = "svmRadial",
           metric = "Kappa",
           trControl = ctrl)
svm_model
svm_pred <- predict(svm_model, nfl)
CrossTable(x = nfl$Drafted, y = svm_pred, prop.chisq = F, dnn = c('actual yes', 'predicted yes'))
confusionMatrix(as.factor(nfl$Drafted), as.factor(svm_pred))
```
...
# customize train() with the control list and grid of parameters 
set.seed(300)
dt_model <- train(as.factor(Drafted) ~ ., data = nfl, method = "C5.0",
           metric = "Kappa",
           trControl = ctrl,
           tuneGrid = grid)
dt_model
dt_pred <- predict(dt_model, nfl)
CrossTable(x = nfl$Drafted, y = dt_pred, prop.chisq = F, dnn = c('actual yes', 'predicted yes'))
confusionMatrix(as.factor(nfl$Drafted), as.factor(dt_pred))
```
...
Combined_prediction <- data.frame(ann_pred, lm_pred, knn_pred, svm_pred, dt_pred, nfl$Drafted)
...
Second_level_DT_model <- C5.0(as.factor(nfl.Drafted) ~ ., data = combined_nfl_train)

Second_level_DT_pred <- predict(Second_level_DT_model, combined_nfl_test)

  confusionMatrix(as.factor(combined_nfl_test$nfl.Drafted), as.factor(Second_level_DT_pred))
...
Second_level_DT_model_with_costs <- C5.0(as.factor(nfl.Drafted) ~ ., data = combined_nfl_train, costs = error_cost)

Second_level_DT_with_costs_pred <- predict(Second_level_DT_model_with_costs, combined_nfl_test)

  confusionMatrix(as.factor(combined_nfl_test$nfl.Drafted), as.factor(Second_level_DT_with_costs_pred))
